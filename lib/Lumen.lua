...
Library.Window = function(self: Library, propertyTable: {})
	...
	PageButtons.Size = UD2(1, 0, 1, -LogoH)
...
Library.SaveConfig = function(Name: string)
	if not HasFS() then
		warn("[bleed.rest] writefile/readfile unavailable in this environment")
		return false
	end
	EnsureFolder()
	...
	if not Ok then
		warn("[bleed.rest] failed to encode config", Encoded)
		return false
	end
	local Path = ConfigPath(Name)
	local WOk, WErr = pcall(writefile, Path, Encoded)
	if not WOk then
		warn("[bleed.rest] writefile failed", WErr)
		return false
	end
	return true
end
...
Library.LoadConfig = function(Name: string)
	...
	if not Exists then
		warn("[bleed.rest] config not found:", Name)
		return false
	end
	local ROk, Raw = pcall(readfile, Path)
	if not ROk then
		warn("[bleed.rest] readfile failed", Raw)
		return false
	end
	...
	if not Ok or type(Data) ~= "table" then
		warn("[bleed.rest] invalid config:", Name)
		return false
	end
...
Library.SetWatermark = function(Text: string?, Enabled: boolean?)
	...
	local CleanText = tostring(Library.Watermark.Text or "bleed.rest")
...
Library.Unload = function()
	...
	Library.Notify({ Title = "bleed.rest"; Text = "Unloaded"; Duration = 2 })
end
...
Library.BuildConfigPage = function(self: Library, Window: any)
	...
	MenuSection:Button({
		...
		Callback = function()
			Library.ToggleMenu(false)
			Library.Notify({ Title = "bleed.rest"; Text = "Menu hidden - press menu key"; Duration = 2 })
		end;
	})
	MenuSection:Button({
		Name = "Unload";
	...
	NotifySection:Button({
		...
		Callback = function()
			Library.Notify({ Title = "bleed.rest"; Text = "This is a test notification"; Type = "Success" })
		end;
	})
...
Library._MountKeySystem = function(Window)
	...
	local function RevealMenu()
		...
		if Library.Auth.WatermarkExpiry and Library.SetWatermark then
			local Base = Library.Watermark.BaseText or Library.Watermark.Text or "bleed.rest"
